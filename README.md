...
Для исправления ситуации требуется завернуть весь UDP трафик по используемому диапазону портов на наш vless сервер. Актуальный диапазон портов на данный момент для UDP (15000-52500), но лучше свериться по ссылке, что ничего не изменилось: [iRacing Support](https://support.iracing.com/support/solutions/articles/31000140706-what-ports-and-ip-addresses-does-iracing-use-). Также необходимо поступить и с [доменными именами](https://support.iracing.com/support/solutions/articles/31000154050-gathering-ping-trace-data-for-troubleshooting).

1. Перейдите в директорию `\etc\xray\configs` и откройте файл **05_routing.json** в любом текстовом редакторе:
2а. Добавьте в него нижеследующий фрагмент кода. Если вы ранее исправляли проблемы с голосом в Discord, то эта секция у вас уже есть и тогда достаточно расширить диапазон портов:
...
2б. Добавьте домены iracing в секцию VPS подключение | доменные имена, такая секция у вас уже есть, поэтому просто добавьте адреса в конец списка, не забудьте, после последнего элемента запятая НЕ НУЖНА!
...
> [!NOTE]
> Мы открываем очень широкий диапазон портов, что может существенно повлияеть на производительность вашего роутера. После включения и запуска iRacing, наблюдайте за загрузкой процессора в админке роутера.
>
> Если роутер перегружен и не справляется, как вариант, использовать отдельный VLESS клиент на ПК.
